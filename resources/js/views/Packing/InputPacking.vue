	...
	<div>
		...
		<div class="row">
			<div class="col-md-12 grid-margin stretch-card">
				<div class="card">
					<div class="card-body">
						<h6 class="card-title">Input Record in <b>Gudang Packing</b></h6>
...
export default {
	...
	methods: {
		...
		async confirmAction() {
			try {
				let response = await axios.patch(
					`/api/gudang-packing/confirm/${this.form.id}`,
					this.form
				);
				if (response.status == 200) {
					this.$toast.success("Confirmed", "Done!", {
		...
		async toggleBusy() {
			...
			let { data } = await axios.get("/api/gudang-packing/input-index");
			this.stocks = [];
