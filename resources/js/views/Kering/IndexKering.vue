		...
		>
			<div class="modal-dialog modal-dialog-centered">
				<div class="modal-content">
					...
					<div class="modal-body">
						<form method="patch" @submit.prevent="CreateProfile">
							<div class="form-group">
								<label for="nop" class="col-form-label">Height (cm)</label>
								<input
									type="number"
						...
						<div class="table-responsive">
							<b-table
								...
								<template #cell(status)="data">
									...
									<template v-if="data.item.status == 'finished on BMI-DB'">
										<span class="badge badge-pill badge-success">
											<b-icon
												class="costum-badge"
												icon="check2-square"
...
export default {
	...
	methods: {
		...
		async cancelMouldingAction() {
			try {
				let response = await axios.patch(
					`/api/gudang-p-kering/cancel-moulding-kering/${this.form.id}`,
					this.form
				);
				if (response.status == 200) {
					this.refreshTable();
		...
		async finishMouldingAction() {
			...
			try {
				let response = await axios.patch(
					`/api/gudang-p-kering/finish-moulding-kering/${this.form.id}`,
					this.form
				);
				if (response.status == 200) {
					this.refreshTable();
		...
		async ProcessTo() {
			...
			try {
				let response = await axios.patch(
					`/api/gudang-p-kering/kering-move-to/${this.form.id}`,
					this.form
				);
				if (response.status == 200) {
		...
		async mouldingAction() {
			...
			try {
				let response = await axios.patch(
					`/api/gudang-p-kering/proceed/moulding/${this.form.id}`,
					this.form
				);
				if (response.status == 200) {
					this.refreshTable();
		...
		JointStage(value) {
			...
			Vue.swal({
				...
				html: `are you sure want move the <b>${this.form.name}</b> - <b>${this.form.tally}</b> to JOINT warehouse ?`,
				icon: "warning",
				confirmButtonText: `Yes`,
				cancelButtonText: "No",
				showCancelButton: true,
		...
		CoatingStage(value) {
			...
			this.form.id = value.id;
			this.form.warehouse_id = value.warehouse_id;

			Vue.swal({
				title: "Next stage is COATING stage",
				html: `are you sure want move the <b>${this.form.name}</b> - <b>${this.form.tally}</b> to COATING warehouse ?`,
				icon: "warning",
				confirmButtonText: `Yes`,
				cancelButtonText: "No",
				showCancelButton: true,
				...
				if (result.isConfirmed) {
					this.CoatingStageAction();
				}
			});
		},
		async CoatingStageAction() {
			console.log(this.form);
			try {
				let response = await axios.patch(
